...
// ── DealerCenter-only inventory ───────────────────────────────────────────────
// DealerCenter is the source of truth for what's in stock. A car Jerry adds shows
// up in the DC feed (→ dc-inventory.json, full records from ma_vehicles + VIN
// decode) hours before CarGurus indexes it. We add those DC-only cars to the site
// immediately so new arrivals don't wait on the CarGurus middleman. Generated by
// operations/build_vehicles_json.py --snapshot, committed by the OAP pull.
const DC_INVENTORY_PATH = resolve(__dirname, '../site/src/data/dc-inventory.json');

function loadDcInventory() {
  try {
    const raw = JSON.parse(readFileSync(DC_INVENTORY_PATH, 'utf8'));
    return raw?.by_vin || {};
  } catch (_) {
    console.warn('  dc-inventory.json not found — DealerCenter-only car overlay unavailable this run.');
    return {};
  }
}
...
async function main() {
  ...
  // ─── DealerCenter-only cars (not yet on CarGurus) ──────────────────────────
  // Add any active DC car the CarGurus scrape doesn't have, built straight from
  // DC + VIN-decoded specs. dealRating stays blank until CarGurus rates it, at
  // which point the normal mapNode path takes over and the badge appears. Manual
  // / URL-stability fields are preserved from any prior record for the VIN.
  const presentVins = new Set(vehicles.map(v => (v.vin || '').toUpperCase()));
  let dcOnlyAdded = 0;
  for (const [vin, rec] of Object.entries(DC_INVENTORY)) {
    if (presentVins.has(vin)) continue;
    const prior = existingByVin[vin] || existingByVin[rec.vin];
    vehicles.push(prior ? {
      ...rec,
      slug: prior.slug || rec.slug,
      dealRating: prior.dealRating || rec.dealRating,
      priceSavings: prior.priceSavings ?? rec.priceSavings,
      dateAdded: prior.dateAdded || rec.dateAdded,
      featured: prior.featured ?? rec.featured,
      sortOrder: prior.sortOrder ?? rec.sortOrder,
      photoPrefix: prior.photoPrefix || rec.photoPrefix,
    } : rec);
    dcOnlyAdded++;
    console.log(`  + DC-only (ahead of CarGurus): ${rec.year} ${rec.make} ${rec.model} ${rec.trim} [${vin}]`);
  }
  if (dcOnlyAdded) {
    console.log(`Added ${dcOnlyAdded} DealerCenter-only car(s) the CarGurus scrape hasn't indexed yet.`);
  }
